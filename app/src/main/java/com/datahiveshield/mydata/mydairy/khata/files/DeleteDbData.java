...
public class DeleteDbData extends Activity {
...
//        String storage_folder = "/."+Params.EmailId+"/"+Params.DB_NAME+"/db";
        ...
        MyDbHandlerGroupConfigs db = new MyDbHandlerGroupConfigs(DeleteDbData.this);
...
//                if(showSelecteDate.getText().toString().length()>21) {
                if(Contact.tempSelectedDate.length()!=0) {
                    if (db.isDateExist(Contact.tempSelectedDate) == 1) {//If selected date exists in table
                        try {
//                    Call MyDbHandlerGroupConfigs method to delete data
                            MyDbHandlerGroupConfigs db = new MyDbHandlerGroupConfigs(DeleteDbData.this);
                            db.deleteData();
                            tableView(db);//Update screen after delete
                            ga.alertDialogBox(DeleteDbData.this, "Date : " + contact.getSelectedDate() + "  record deleted", "Record Deleted");


                        } catch (Exception e) {
                            //Log.d("dbsvm","Unable to delete selected record");
//                            ga.alertDialogBox(DeleteDbData.this, "Unable to delete record for Date : " + contact.getSelectedDate(), "Unable To Delete");
                        }
                    } else {
                        ga.alertDialogBox(DeleteDbData.this, "Record does not exist with selected  " + "Date : " + Contact.tempSelectedDate + "\nPlease select valid date", "Invalid date selected");
//                    ga.alertDialogBox(DeleteDbData.this,"Please select appropriate date to delete","Not valid selection");
                    }
                }else {
...
//            public void onClick(View view) {
//                if(showSelecteDate.getText().toString().length()>21) {
//                    startActivity(new Intent(DeleteDbData.this, UpdateDbData.class));
//                }
//                else
//                {
//                    GetAlerts ga=new GetAlerts();
//                    ga.alertDialogBox(DeleteDbData.this,"Please select appropriate date to update");
//                }
//            }
//        });
//   }
...
//            MyDbHandlerGroupConfigs db = new MyDbHandlerGroupConfigs(com.easyway2in.sqlitedb.TableView.this);
            table.removeAllViews();//to update the same table orientatin after delete
            //on second call of tableView from delete button, first clear whole data
            //then rewrite on same postions after data deleted
