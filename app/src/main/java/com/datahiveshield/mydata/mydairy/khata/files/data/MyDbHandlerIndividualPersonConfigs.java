...
public class MyDbHandlerIndividualPersonConfigs extends SQLiteOpenHelper {
    public static SQLiteDatabase db1;
    public Context context;

    public MyDbHandlerIndividualPersonConfigs(Context context)
    {
...
//        super(context, Params.DATATABSE_DIRECTORY_NAME,null,Params.DB_VERSION);
        super(context, context.getExternalFilesDir(null).getAbsolutePath()
                + File.separator + "/."+Params.EmailId+"/"+Params.DEFAULT_INDIVIDUAL_PERSON_DATA_NAME+"/"+Params.youWillGetOrGiveMoneyButtonPressed+"/"+ Params.CURRENT_GROUP_DB_NAME+"/"+Params.CURRENT_GROUP_CONFIG_DB_NAME
                + File.separator
                + Params.DB_NAME,null,Params.DB_VERSION);

//        Log.d("dbsvm","Current group config db_name - "+Params.CURRENT_GROUP_CONFIG_DB_NAME);
...
//
    ...
    public void createIndividualPersonDataValueStoringTable(SQLiteDatabase db)//Main table to store value data of customers
    {
        try {
            String create = "CREATE TABLE IF NOT EXISTS " + Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME + "(" +
                    Params.DATE + " VARCHAR(10) PRIMARY KEY  , " +
                    Params.C1 + " DECIMAL(5,2) NOT NULL, " + Params.C2 + " VARCHAR(10) NOT NULL , " +
                    Params.C3 + " VARCHAR(50) NOT NULL  "   + ")";
...
//
    ...
    public void insertIndividualPersonData(IndividualPersonData contact)//used only once inside insert button InsertDataDb.java
    {
        try {
...
//            //Log.d("sk", "Successfully inserted");
            ...
            values.put(String.valueOf(Params.C1), contact.getPERSON_DATA());
            values.put(String.valueOf(Params.C2), contact.getStatus());
            values.put(String.valueOf(Params.C3), contact.getADD_NOTE());

            db.insert(Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME, null, values);
...
//            return 0;
    ...
    public int isIndividualPersonDataDateExist(String date) {
...
//            hasObject = true;
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public List<IndividualPersonData> getAllIndividualPersonDataSavedRecords(){
        List<IndividualPersonData> contactList=new ArrayList<>();
        SQLiteDatabase db=this.getReadableDatabase();
        //Generate the query to read from database
        String select="";
        try {
            select = "SELECT * FROM " + Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME;
        }
        catch (Exception e)
        {
            Log.d("dbsvm","unable to fetch unpaid records\nError :"+e.getMessage());
        }
//        String select="SELECT * FROM "+Params.DEFAULT_DATA_TABLE_NAME;
        ...
        {
            do{
                IndividualPersonData contact=new IndividualPersonData();
                contact.setDATE(cursor.getString(0));
                contact.setPERSON_DATA(Double.parseDouble(cursor.getString(1)));
                contact.setSTATUS(cursor.getString(2));
                contact.setADD_NOTE(cursor.getString(3));
//                Log.d("dbsvm",contact.getDATE()+"  "+contact.getPERSON_DATA()+" "+contact.getStatus()+"  "+contact.getADD_NOTE());
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public String getIndividualPersonAmountNotPaidRecords(){
        String amountNotPaid="0";
        SQLiteDatabase db=this.getReadableDatabase();
        //Generate the query to read from database
        String select="";
        try {
            select = "SELECT SUM( " + Params.C1 + " ) FROM " + Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME + " WHERE " + Params.C2 + " = 'Not Paid'";
        }
        catch (Exception e)
        {
            Log.d("dbsvm","unable to fetch unpaid records\nError :"+e.getMessage());
...
//        String select="SELECT * FROM "+Params.DEFAULT_DATA_TABLE_NAME;
        ...
        if(cursor.moveToFirst())
        {
                amountNotPaid=cursor.getString(0);
//                contact.setPERSON_DATA(Double.parseDouble(cursor.getString(1)));
        }

        Log.d("dbsvm","Individual amountNotPaid : "+amountNotPaid);
        try
        {
            if(amountNotPaid.equals(null))//here it will throw exception for comparing
//                on null value reference , so we will update null to 0 in catch block
            {
//                amountNotPaid="0";
                Log.d("dbsvm","Individual amountNotPaid is null: "+amountNotPaid);
        ...
        {
            amountNotPaid="0";
            Log.d("dbsvm","Inside MyDbHandlerIndividualConfigs :: getIndividualPersonAmountNotPaidData method " +
                    "amountNotPaid is null so replaced with 0 in catch block" +
                    " individual data\nError : "+e.getMessage());
        }
//        Log.d("dbsvm","Individual amountNotPaid : "+amountNotPaid);
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public String getIndividualPersonAmountPaidRecords(){
        String amountPaid="0";
        SQLiteDatabase db=this.getReadableDatabase();
        //Generate the query to read from database
        String select="SELECT SUM( "+Params.C1 +" ) FROM "+Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME+" WHERE "+Params.C2+" = 'Paid'";
//        String select="SELECT * FROM "+Params.DEFAULT_DATA_TABLE_NAME;
        ...
        if(cursor.moveToFirst())
        {
            amountPaid=cursor.getString(0);
//                contact.setPERSON_DATA(Double.parseDouble(cursor.getString(1)));
        }

        Log.d("dbsvm","With query, amount paid "+amountPaid);
        return amountPaid;
    }
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public IndividualPersonData getSavedRecordForIndividualPersonSelectedDate(String sdate){
        IndividualPersonData contact=new IndividualPersonData();
        SQLiteDatabase db=this.getReadableDatabase();
        //Generate the query to read from database
//        String select="SELECT * FROM "+Params.DEFAULT_DATA_TABLE_NAME;

        Cursor cursor=db.rawQuery("SELECT * FROM "+Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME + " WHERE "+ Params.DATE+ " =?", new String[]{sdate});
...
//                Log.d("dbsvm",contact.getDATE()+"  "+contact.getPERSON_DATA()+" "+contact.getStatus()+"  "+contact.getADD_NOTE());
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public void updateIndividualPersonData(IndividualPersonData contact)
    {
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            ContentValues values = new ContentValues();
...
//            db.update(Params.TABLE_NAME, values, "",new String[]{contact.getSelectedDate()});
            db.update(Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME, values, Params.DATE + "=?" ,
...
//            db.insert(Params.TABLE_NAME, null, values);
    ...
    //update whole table status to paid
    @RequiresApi(api = Build.VERSION_CODES.O)
    public void paidAllIndividualPersonData()
    {
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            ContentValues values = new ContentValues();
...
//            values.put(Params.C2, contact.getC2());
            ...
            values.put(String.valueOf(Params.C2), "Paid");

//            db.update(Params.TABLE_NAME, values, "",new String[]{contact.getSelectedDate()});
            db.update(Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME, values,null,null);
...
//            db.insert(Params.TABLE_NAME, null, values);
    ...
    //update whole table status to Not Paid
    @RequiresApi(api = Build.VERSION_CODES.O)
    public void unpaidAllIndividualPersonData()
    {
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            ContentValues values = new ContentValues();
...
//            values.put(Params.C2, contact.getC2());
            ...
            values.put(String.valueOf(Params.C2), "Not Paid");
...
//            db.insert(Params.TABLE_NAME, null, values);
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public void deleteIndividualPersonData() {
        try {
            SQLiteDatabase db = this.getWritableDatabase();
...
//            SQLiteDatabase db = this.getWritableDatabase();
            db.delete(Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME, Params.DATE +"=?", new String[]{String.valueOf(Contact.selectedDate)});
            db.close();
...
//            db.execSQL("DROP TABLE IF EXISTS "+Params.TABLE_NAME);//Table deleted
    ...
    public void clearIndividualPersonDataTable() {
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.execSQL("delete from " + Params.INDIVIDUAL_PERSON_DATA_TABLE_NAME);
//            db.execSQL("DROP TABLE IF EXISTS "+Params.TABLE_NAME);//Table deleted
            //Log.d("dbsvm", "table data cleared");
            db.close();
        }
...
//        db.execSQL("DROP TABLE IF EXISTS "+Params.DEFAULT_CUSTOMER_NAMES_TABLE);
    ...
    public void deleteIndividualPersonConfig(String dbName)//Working fine group name deleted successfully
    {
//        SQLiteDatabase db = this.getWritableDatabase();
...
//        String path = context.getExternalFilesDir(null).getAbsolutePath() + "/."+Params.EmailId+"/" + Params.CURRENT_GROUP_DB_NAME;
        String path = context.getExternalFilesDir(null).getAbsolutePath() + "/."+Params.EmailId+"/"+Params.DEFAULT_INDIVIDUAL_PERSON_DATA_NAME+"/"+Params.youWillGetOrGiveMoneyButtonPressed+"/" + Params.CURRENT_GROUP_DB_NAME + File.separator + dbName;
